...
// --- Main Tool ---
server.tool("get-linear-tickets", "Fetches Linear tickets based on a description.", { description: z.string().describe("A description of the feature, bug, or topic to search for in Linear tickets.") }, async ({ description }) => {
    ...
    try {
        ...
        const formattedTickets = tickets.map((ticket) => (`Ticket ID: ${ticket.identifier}\n` +
            `Title: ${ticket.title}\n` +
            `Status: ${ticket.state?.name || 'N/A'}\n` +
            `Assignee: ${ticket.assignee?.name || 'Unassigned'}\n` +
            `Project: ${ticket.project?.name || 'N/A'}\n` +
            `URL: ${ticket.url}\n` +
            `Description: ${ticket.description ? ticket.description.substring(0, 200) + '...' : 'No description'}`)).join("\n\n---\n\n");
